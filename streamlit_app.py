...
if df is not None:
    ...
    # --- Persist cleaned df back to session state (so downstream widgets always see the latest version) ---
    ...
    if not feature_cols:
        ...
        if task_type == "Classification":
            ...
            if len(class_counts) < 2:
                ...
                if imbalance_ratio < 0.5:
                    ...
                    if smote_option:
                        try:
                            # Robust SMOTE: dynamically set k_neighbors
        ...
        if feature_cols and (
            (task_type == "Regression") or (task_type == "Classification" and len(Counter(y_train)) > 1)
        ):
            # --- Model Selection ---
            if task_type == "Classification":
                model_options = [
                ...
                # Remove XGBoost if not installed
...
# Set your Azure endpoint and model
